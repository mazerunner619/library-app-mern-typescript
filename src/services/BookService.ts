...
export const findAllBooksLimited = async(limit:number, page:number, totalCount:number):Promise<IPagination<IBookModel>> => {
    try {
        ...
        const books = await BookDao.find().skip((page-1)*limit).limit(limit);
...
export const modifyBook = async(book : IBookModel):Promise<IBookModel> => {
    try {
        const updatedBook = await BookDao.findOneAndUpdate({barcode: book.barcode}, book, {new:true});
        if(updatedBook)
            return updatedBook;
...
export const queryBooks = async(page:number, limit: number, totalCount: number, title?:string, barcode?:string, description?:string, author?:string, subject?:string, genre?:string):Promise<IPagination<IBookModel>> => {
    try {
        const filter = [];
        ...
        if(title && title !== '')
            filter.push({title : new RegExp(title, 'i')});
        ...
        if(genre && genre !== '')
            filter.push({genre : new RegExp(genre, 'i')});
...
export const paginateBooks = (books:IBookModel[], page:number, limit:number):IPagination<IBookModel> => {
