...
export const findAllBooksLimited = async (
  ...
  try {
    ...
    if (totalCount === -1) totalCount = await BookDao.countDocuments();
    totalPages = Math.ceil(totalCount / limit);

    // to fetch books from redis cache
    // const redisQueryKey = `books:limit-${limit}page-${page}`;
    // const books = await getOrSetCache<IBookModel[]>(redisQueryKey, async () => {
...
export const queryBooks = async (
  ...
  try {
    ...
    totalPages = Math.ceil(totalCount / limit);
    // const books = await getOrSetCache<IBookModel[]>(searchKey, async () => {
    //   return await BookDao.find({
    //     $or: filter,
    //   })
    //     .skip(limit * (page - 1))
